...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "LearnLeader Reflecting…"
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "LearnLeader Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "LearnLeader";
  ...
  else if (mode === "responded") modeLabel = t("LearnLeader responded.");
 ...
 * ("LearnLeader Exploring… · 8s", settling to "LearnLeader responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while LearnLeader is still reasoning /
