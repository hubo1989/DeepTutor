...
function SessionViewerPanelInner(
  ...
  /* Below the desktop breakpoint the panel is a full-screen sheet, not a side
     panel: there is no chat column left to sit beside, and VIEWER_WIDTH_MIN is
     wider than a phone or portrait tablet. That override lives in CSS
     (`max-lg:!w-full`) rather than `useDevice()` so it is right on the first
  ...
  return (
    <div
      role="dialog"
      aria-hidden={!visible}
      className={`fixed right-0 top-0 z-[30] flex h-dvh flex-col border-l border-[var(--border)] bg-[var(--card)] transition-transform ease-out max-lg:!w-full lg:max-w-[92vw] ${
    ...
    >
      ...
      <div
        ...
        className="group/resize absolute left-0 top-0 z-10 h-full w-2 -translate-x-1/2 cursor-col-resize max-lg:hidden"
      >
