...
def test_first_account_is_promoted_when_no_env_admin_exists(mu_isolated_root):
    # LearnLeader: save_user never implicitly promotes the first account —
    # admin authority comes from the email-verification flow (first verified
    # user) or the admin panel, so a bare store stays caller-controlled.
    from deeptutor.multi_user.identity import save_user
...
def test_partial_env_credentials_do_not_count_as_an_admin(mu_isolated_root, monkeypatch):
    ...
    # LearnLeader: partial bootstrap credentials cannot adopt an account, and
    # no implicit first-account promotion exists to un-block the deployment;
    # the first *email-verified* registration is promoted instead.
    assert record["role"] == "user"
