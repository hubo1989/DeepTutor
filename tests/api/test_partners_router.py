...
class TestChannelOnboarding:
    ...
    def test_onboarding_routes_carry_the_partner_manage_gate(self, isolated_root, monkeypatch):
        ...
        # LearnLeader's decode_token revalidates the account store (existence,
        # email_verified, auth_version), so the token must belong to a real
        # user record for this assertion to observe the manage gate (403/404)
        # rather than a bare 401.
        from deeptutor.multi_user.identity import save_user

        save_user("not-an-admin", "irrelevant-hash", role="user")
