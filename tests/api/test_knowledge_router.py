...
class _FakeKBManager:
    ...
    def get_knowledge_base_path(self, name: str) -> Path:
        # Path resolution only: the real manager never creates directories
        # here, and the upload route must not materialize storage for a KB it
        # is about to reject as read-only.
    ...
    class _CountingKBManager:
        ...
        def get_info(
            ...
            *,
            ...
            # The real manager's get_info() lazily loads kb_config.json into
            # ``manager.config``; the /list route reads that attribute right
            # after calling get_info, so mirror the side effect.
