...
def test_build_openai_client_survives_stale_ssl_cert_file(
    ...
    try:
        # The underlying httpx transport initialised successfully. LearnLeader
        # wraps clients in the token-quota layer, so reach through the OpenAI
        # SDK handle down to its httpx transport.
        openai_handle = client._client
        httpx_client = getattr(openai_handle, "_client", openai_handle)
        assert getattr(httpx_client, "_transport", None) is not None
    finally:
